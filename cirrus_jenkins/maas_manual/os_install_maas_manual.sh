...
if [  -z $password ]
then
	echo "The password value for user creation is set to null exiting the script"
	exit 2
else
          ...
          if [ $? -eq 0 ]; then
		echo "MAAS login is working fine "
		cat /dev/null/ &> /tmp/server1_list
		if [ -z $server_list ]
		then
			echo " Server list not provided by user, system will deploy os on all nodes in ready and released state"
		...
		echo -e " list of servers for OS deployment \n `cat $server1_list`"
			...
			ready_count=`cat /tmp/ready_server | wc -l`
                        if  [ $ready_count -gt 0 ]
                               ...
                               for system_id in `cat /tmp/ready_server| awk -F "," '{print $3}'`
                                       do
                               		echo " allocating system with system_id = $system_id  and  hostname =`cat /tmp/ready_server| grep -i $system_id | awk -F "," '{print $1}'`"
...
#                               sudo  maas $maas_admin machine update $system_id pool=$pool
                       ...
                       # fi
			echo "Waiting for completion of deployment for 15 minutes "
			sleep 900
			...
			touch /tmp/deploy_server1
					...
					sudo maas $maas_admin machine  read $system_id | jq -r ' .hostname + "," +.status_message + "," + .system_id + "," + .hardware_info.system_vendor'|  grep -i Deployed >> /tmp/deploy_server1
					done
			...
			then
			echo -e " Deploy count is less than the total enlistd server count detected initially"
			...
			do
				...
				count=count+1
                        	echo "Loop count: $count , sleeping system for 1 minnutes"
                                echo " waiting for $server_count  servers deployment to be completd"
				sleep 60
			done
			else
				echo " $deploy_count1 servers are now deployed"
			fi
					...
					do 
					  ...
					  deploy_count=`cat /tmp/deploy_server| wc -l`
                                	echo " Deployed server count: $deploy_count and Total server count: $server_count and loop count: $count"
					
                             #		sleep 60
					done
					if [ $deploy_count -gt 0 ]
					then	
					echo -e "OS deployment on the below nodes is completed, you copy paste the below list in server_list input of next pipeline to configure ssh keys and netplan \n`cat /tmp/deploy_server| grep -i deployed`\n\n" 
					else 
					echo "failed to deploy all the nodes"
					exit 2
					fi
		...
		for host_info in `cat /tmp/deploy_server| grep -i deployed |  awk -F "," '{print $1}'`
			...
			echo "the file with list of nodes $server1_list is not present"
			exit 2
	  	fi
	else 
